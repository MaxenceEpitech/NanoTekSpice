 ...
 ** NanoTekSpice2
 ** File description :
 ** 	File Name : C4001.cpp Author : maxence.fourrier Date : 2 mars 2018
 */
...
nts::C4001::C4001()
		: _size(13), _name("4001"), _nbGates(4)
{
	...
	_gates[0] = nts::LogicalGate(nts::GateType::NOR, &_pin[0], &_pin[1], &_pin[2]);
	_gates[1] = nts::LogicalGate(nts::GateType::NOR, &_pin[4], &_pin[5], &_pin[3]);
	_gates[2] = nts::LogicalGate(nts::GateType::NOR, &_pin[7], &_pin[8], &_pin[9]);
	_gates[3] = nts::LogicalGate(nts::GateType::NOR, &_pin[12], &_pin[11], &_pin[10]);
}
...
nts::Tristate nts::C4001::compute(std::size_t pin) const
{
	if (pin >= _size) {
...
nts::Pin* nts::C4001::getPinById(size_t id)
{
	return &(this->_pin[id]);
}
