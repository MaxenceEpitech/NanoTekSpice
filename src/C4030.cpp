 ...
 ** NanoTekSpice2
 ** File description :
 ** 	File Name : C4030.cpp Author : maxence.fourrier Date : 2 mars 2018
 */
...
nts::C4030::C4030()
		: _size(13), _name("4030"), _nbGates(4)
{
	...
	_gates[0] = nts::LogicalGate(nts::GateType::XOR, &_pin[0], &_pin[1],
			&_pin[2]);
	_gates[1] = nts::LogicalGate(nts::GateType::XOR, &_pin[4], &_pin[5],
			&_pin[3]);
	_gates[2] = nts::LogicalGate(nts::GateType::XOR, &_pin[7], &_pin[8],
			&_pin[9]);
	_gates[3] = nts::LogicalGate(nts::GateType::XOR, &_pin[11], &_pin[12],
			&_pin[10]);
}
...
nts::Tristate nts::C4030::compute(std::size_t pin) const
{
	if (pin >= _size) {
...
nts::Pin* nts::C4030::getPinById(size_t id)
{
	return &(this->_pin[id]);
}
